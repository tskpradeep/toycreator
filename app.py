...
# Helper to load config for the AI call
def get_config():
...
.btn-cell{{ aspect-ratio:1/1; width:20px; height:20px; background:#e1e1e1; color:#000; border:2px solid #fff; border-right:2px solid #707070; border-bottom:2px solid #707070; cursor:pointer; display:flex; align-items:center; justify-content:center; box-sizing:border-box; }}
...
.text-main{{ color:#b22222; font-size:1.4vw; font-weight:bold; text-align:center; width:100%; height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; }}
...
<div class="master-container">
        ...
        <div id="split-container" style="display:flex;flex:1;width:100%;">
            <div id="left-stack" style="display:flex;flex-direction:column;width:70%;">
                <div id="cad-pane" class="pane text-main">
                    <div id="visual-monitor">[ IDLE: AWAITING CIRCUIT REQUEST ]</div>
                </div>
    ...
    <div class="fixed-footer">
        ...
        <div class="selection-b-container">
            <div class="dropup tall"><span>ACTIVE: {config.get('version', 'NONE')}</span><span>▲</span></div>
        </div>
    </div>
</div>
...
async function callGemini(promptText){{
    ...
    try{{
        terminal.innerHTML += "\\n> API_CALL: HANDSHAKE";
        const response = await fetch(`${{config.api_url}}${{config.version}}:generateContent?key=${{config.api_key}}`,{{
            method:"POST",
...
document.getElementById('user-prompt').addEventListener('keydown',function(e){{
    if(e.key==='Enter' && !e.shiftKey){{
        ...
        if(text){{
            document.getElementById('ai-chat').innerHTML += "<br><b>[USER]:</b> "+text;
            callGemini(text);
            this.value='';
        }}
    }}
...
"""
components.html(cad_app_html, height=900)
