...
<div class="master-container">
    ...
    <div id="ai-modular-setup">
        <div class="ai-setup-header">
            ...
            <div>
                <button onclick="saveData()" style="color:#00ff00; background:0; border:1px solid #00ff00; cursor:pointer; padding:2px 10px;">SAVE</button>
                <button onclick="toggleAISet(false)" style="color:#fff; background:0; border:1px solid #fff; cursor:pointer; margin-left:10px; padding:2px 10px;">[ X ]</button>
            </div>
        </div>
        <div class="ai-setup-body">
            ...
            <div class="ai-setup-content">
                <div style="font-size: 16px; color:#fff;">DIRECT MODEL SELECTION:</div>
                <select class="ai-select" id="version-select">
                    <option value="gemini-1.5-pro">Gemini 1.5 Pro (Stable High Reasoning)</option>
                    <option value="gemini-1.5-flash">Gemini 1.5 Flash (Performance Tier)</option>
                </select>
                <div>
                    <label>API KEY (Required):</label>
                    <input type="password" id="api-field-input" class="ai-input" placeholder="Enter Official Gemini Key...">
                </div>
    ...
    <div class="window-title-bar">
        <div>CAD DESIGNER PRO - INTEGRITY BUILD</div>
    ...
    <div id="dynamic-zone">
        <div id="left-stack" style="display:flex; flex-direction:column; width:70%;">
            <div id="cad-pane" class="pane" style="flex:4;">
                <div id="visual-monitor" style="color: #444; font-size: 12px; font-family: monospace;">[ SYSTEM IDLE ]</div>
            </div>
        ...
        <div id="right-stack" style="display:flex; flex-direction:column; width:30%;">
            <div id="ai-output" class="pane" style="flex:1;">
                <div id="ai-chat" class="ai-text-area">ENTER KEY IN AI-SET TO START</div>
            </div>
    ...
    <div class="fixed-footer">
        <div style="flex:1; display:flex; align-items:center; color:#00ff00; font-size:10px; padding-left:10px;">SESSION STATUS: ACTIVE</div>
        <div style="width:130px; background:#e1e1e1; color:#000; text-align:center; cursor:pointer; line-height:60px; font-size:14px; font-weight:bold;" onclick="toggleAISet(true)">AI-SET ▲</div>
    </div>
</div>
...
<script>
    ...
    async function callGemini(promptText) {
        ...
        if (!apiKey || apiKey === "") { 
            chatWindow.innerHTML += "<br><span style='color:red'>[ERROR]: NO API KEY FOUND.</span>"; 
            return; 
        }

        try {
            terminal.innerHTML += "\\n> API_REQUEST: " + model.toUpperCase();
            ...
            chatWindow.innerHTML += `<br><span style="color:red">[API ERROR]: ${err.message}</span>`;
            terminal.innerHTML += "\\n> API_FAILURE: REJECTED";
        }
    }
